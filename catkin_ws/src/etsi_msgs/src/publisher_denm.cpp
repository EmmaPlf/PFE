...
void odomCallback(const sensor_msgs::NavSatFix::ConstPtr &navSat,
                  struct navSat *navSat_arg) {

  ROS_INFO("I have received a message odom");
  navSat_arg->lat = navSat->altitude;
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber sub = n.subscribe<sensor_msgs::NavSatFix>(
      "fix", 1000, boost::bind(odomCallback, _1, &navSat_arg));
 ...
 */
  ...
  // OPTIONAL
  // The traffic direction along which the event information is relevant for the
  // receiving ITS-S, as defined in clause 6.1.3.1. RelevanceTrafficDirection_()
  // : value(UNAVAILABLE) {}
  ...
  // event is detected.
  msg.management.detection_time =
      eventTime_arg.detect; // ms since 2004 uint64_t
  ...
  msg.situation.event_type.sub_cause_code = situation_arg.sub_cause; // uint8_t

  // OPTIONEL
  // LinkedCause: This DF indicates an event which may be linked with the
  // eventType. It shall be as defined in clause B.26. The linkedCause is an
  // optional DF. It shall be present in the situation container, if the
  // application provides such information to the DEN basic service.
  // msg.situation.has_linked_cause = false;

  // OPTIONEL
  // CauseCodeType ::= INTEGER { reserved (0), trafficCondition
  // (1), accident (2), roadworks (3), adverseWeatherCondition-Adhesion (6),
  // hazardousLocation-SurfaceCondition (9), hazardousLocation-ObstacleOnTheRoad
  // (10), hazardousLocation-AnimalOnTheRoad (11), humanPresenceOnTheRoad (12),
  // wrongWayDriving (14), rescueAndRecoveryWorkInProgress (15),
  // adverseWeatherCondition-ExtremeWeatherCondition (17),
  // adverseWeatherCondition-Visibility (18),
  // adverseWeatherCondition-Precipitation (19), slowVehicle (26),
  // dangerousEndOfQueue (27), vehicleBreakdown (91), postCrash (92),
  // humanProblem (93), stationaryVehicle (94),
  // emergencyVehicleApproaching (95), hazardousLocation-DangerousCurve (96),
  // collisionRisk (97), signalViolation (98), dangerousSituation (99) }
  // (0..255)
  // msg.situation.linked_cause.cause_code = 0;     // uint8_t
  // msg.situation.linked_cause.sub_cause_code = 0; // uint8_t

  // OPTIONAL
  // The DF consists of a list of event points which represents the dimension of
  // a plain event in a predefined order. In case that the plain event is
  // detected by a vehicle ITS-S, the DF consists of a list of event detection
  // points along the path that the detecting ITS-S has travelled over some past
  // time and/or distance. Each event point corresponds to a point at which the
  // same event was detected along the path.
